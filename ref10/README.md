...
__NOTE__: The SUPERCOP benchmark assumes a Linux system and so does this guide. The steps explain how to extract the required C source code and how to compile it into static libraries and shared object files. The source code is mostly os-independent, only the kernelrandombytes module uses os-specific code to retrieve cryptographically secure random data. Any Windows dev with a bit of experience should be able to build a DLL from the source code.

__NOTE__: The ref10 code uses 64-bit integers in a few places. On architectures with less than 64-bit registers this results in more and slower code but should generally work the same. I tested compiling the library for 32-bit using gcc's `-m32` flag on my 64-bit machine.
...
A couple of files that define fixed-size integers are missing. Those are located inside of your data directory, following the path `inttypes/include/<architecture>`.

Now just two more header files are missing: `crypto_scalarmult.h` and `crypto_scalarmult_curve25519.h`. The benchmark runs each module with a set of different compiler arguments, e.g. with different optimization levels. It creates a unique directory for each of the configurations, located inside of your data directory. The benchmark puts additional generated header files into these directores, including the two headers for the crypto_scalarmult module. The full path to the header files looks like this: `<architecture>/try/c/<configuration>/crypto_scalarmult/curve25519/ref10/compiled`. It does not matter which architecture or configuration you look at, the headers are the same for all of them. This guide uses the `amd64` architecture and the `gcc` configuration.

These are the three locations that contain all the code required to build the static library and the shared object file. Either copy all files into one directory or tell your compiler where to look for the files.
...
$ gcc -Wall -Werror -g -shared -Wl,--no-undefined -o libcrypto_scalarmult.so *.o
...
__NOTE__: To keep things simple, this guide only shows, how to pack the dependencies into static libraries. Because static libraries are not linked, static libraries do not check refernces at build-time, this allows us to process each dependency on its own without having to care about the order and other dependencies.
...
$ gcc -Wall -Werror -g -shared -Wl,--no-undefined -o libcrypto_sign.so *.o -lcrypto_hash -lcrypto_hashblocks -lcrypto_verify -lfastrandombytes -lkernelrandombytes -lcrypto_rng -lcrypto_stream -lcrypto_core
```

Or package another static library the same way as for the dependencies:

```bash
$ ar rcs libcrypto_sign.so *.o
```

__NOTE__: When using the static library, you have to link ALL the static libraries of ALL recursive dependencies, e.g.:
...
This section will address the topic of making the code portable/platform-independent.
...
In the same way all int-type headers can be refactored, using following type-table:
...
Notice that this does not handle a few possible failures:

- `LoadLibrary` could fail. This should only happen on systems older than Windows XP SP3.
- `GetProcAddress` could fail. Again, only happens on systems older than Windows XP SP3.
- `RtlGenRandom` could fail. The documentation of `RtlGenRandom` does not state, why the function could fail. Thus, the possible failure gets ignored in the example above.

### 3.2.2 Mac OS

On Mac OS, the go-to random API is `SecRandomCopyBytes`, which is available as a simple C function. The following shows an implementation of `kernelrandombytes` using the function:
...
That should be it! Secure random bytes for all major OS families.

### 3.3. Cross-platform dynamic libraries

The current guide uses `gcc` to compile the sources into shared object files and `ar` to create static libraries.

This section covers the required commands to build static and dynamic versions of the libraries on Windows and Mac.

### 3.3.1 Windows

On Windows, [MinGW](http://www.mingw.org/) offers an open source `gcc` environment, which allows to use slightly modified UNIX commands to compile Windows-native static and shared libraries.
